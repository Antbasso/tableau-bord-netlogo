...
to Go
  ...
  let Orden.test false ; true o False
  if(Orden.test = true)[
...
to-report production-operations
  let ops []
  ask machines [
    set ops lput (list Machine.Name Machine.Operations.Type Machine.Operations.Time) ops
  ]
  report ops
end
...
to-report get-production-time [product-id]
  let p product product-id

  if p = nobody [
    report -1  ; Produit non trouvé
  ]

  let start [productrealstart] of p
  let completion [productrealcompletion] of p

  ifelse (start != 0) and (completion != 0) [
    report completion - start
  ] [
    ifelse start != 0 [
      report ticks - start  ; Temps écoulé jusqu'à présent
    ] [
      report -1  ; Pas encore commencé
    ]
  ]

end

to-report system-snapshot
  let machines-data [ (list Machine.Name Machine.State Next.Completion Machine.Operations.Type Machine.Operations.Time xcor ycor heading) ] of machines
  let products-data [ (list Product.State ProductType Next.Product.Operation ProductOperations CurrentSequenceOrder Next.Product.Completion.Time xcor ycor heading) ] of products
  report (list machines-data products-data)
end
...
242
A
if(Time-for-Possible-launching = 0)\n[create.product \"A\" \nset Time-for-Possible-launching 100\n]
NIL
1
T
OBSERVER
